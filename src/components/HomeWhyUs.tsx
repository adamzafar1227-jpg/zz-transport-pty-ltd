...
import { Users, MapPin, ShieldCheck, Award, HeartHandshake, Zap } from "lucide-react";
...
function StatCounter({ target, suffix, subtext, description, icon: IconComponent }: StatCounterProps) {
  ...
  return (
    ...
    >
      ...
      {/* Icon circle */}
      <div className="w-12 h-12 bg-[#1A1A1A] rounded-lg flex items-center justify-center mb-4 text-[#F5A623] border border-[#F5A623]/10 group-hover:scale-110 transition-transform">
        <IconComponent className="h-6 w-6" />
      </div>

      <div className="font-display font-black text-4xl sm:text-5xl text-[#F5A623] pb-1 tracking-tight">
        {count}
        {suffix}
      </div>
...
export default function HomeWhyUs() {
  ...
  return (
    <section id="why-us" className="py-24 bg-[#0A0A0A] text-white relative border-b border-[#1F1F1F]">
      {/* Decorative side block */}
      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-32 bg-[#F5A623] rounded-r-full hidden lg:block" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-16">
          <div>
            <span className="text-[#F5A623] font-mono text-xs font-bold uppercase tracking-widest block mb-3">
              - Why Client Partners Choose Us
            </span>
            <h2 className="font-display font-[900] text-3xl sm:text-4xl text-white tracking-tight uppercase" id="why-us-title">
              Why Choose ZZ Transport
            </h2>
            <div className="w-20 h-1.5 bg-[#F5A623] mt-4 rounded-full" />
          </div>
          <div>
        ...
        <div className="mt-16 pt-8 border-t border-white/5 flex flex-wrap justify-center items-center gap-8 text-white/40">
          <div className="flex items-center gap-2 text-sm font-sans">
            <ShieldCheck className="h-5 w-5 text-[#F5A623]/65" />
            <span>NHVR Approved Fleet</span>
          </div>
          <div className="flex items-center gap-2 text-sm font-sans">
            <Zap className="h-5 w-5 text-[#F5A623]/65" />
            <span>Customs Compliant Facilities</span>
          </div>
          <div className="flex items-center gap-2 text-sm font-sans">
            <Users className="h-5 w-5 text-[#F5A623]/65" />
